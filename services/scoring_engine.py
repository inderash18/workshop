...
def compute_scores(candidate, submission):
    candidate_id = candidate.get("candidate_id", "")
    levels = get_challenge_data(candidate_id)

    answers_by_level = submission.get("answers", {})
    ...
    if violation_count >= 3:
        ...
        violation_logs.append({
            ...
            "detail": f"{violation_count} violations exceeded limit",
        })
    else:
    ...
    if typing_speed_avg > 1500:
        long_text = any(
            len(text_answers_by_level.get(lid, [""])[0]) > 100
            for lid in text_answers_by_level
        )
        if long_text:
            violation_logs.append({
                "timestamp": datetime.now().isoformat(),
